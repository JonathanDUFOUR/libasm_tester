...
mod tests {
	...
	fn unit_test_helper(s: &[u8], base: &[u8], expected: i32) {
		let s: &CStr = CStr::from_bytes_with_nul(s).unwrap();
		let base: &CStr = CStr::from_bytes_with_nul(base).unwrap();

		assert_eq!(unsafe { ft_atoi_base(s.as_ptr(), base.as_ptr()) }, expected);
	}
	...
	fn ft_atoi_base_00() {
		...
		unit_test_helper(&[0xa6, 0xec, 0x9d, 0x4c, 0xa2, 0xc4, 0xd0, 0x00], &[0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_01() {
		...
		unit_test_helper(&[0x20, 0x01, 0x7d, 0xa2, 0xb3, 0x00], &[0x01, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_02() {
		...
		unit_test_helper(
			&[0x56, 0x7e, 0xb1, 0x9a, 0xd0, 0x80, 0x90, 0x6a, 0x00],
			&[0x95, 0x4b, 0x56, 0x95, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_03() {
		...
		unit_test_helper(&[0x2b, 0x3b, 0xc6, 0xdd, 0x00], &[0xc6, 0x3b, 0x45, 0x2b, 0x9b, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_04() {
		...
		unit_test_helper(
			&[0x0c, 0x0c, 0x2b, 0xa3, 0x20, 0x00],
			&[0xb6, 0x2d, 0xb7, 0xa8, 0xa3, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_05() {
		...
		unit_test_helper(
			&[0x09, 0x0c, 0xa1, 0x00],
			&[0x09, 0xa1, 0xd0, 0xfc, 0xda, 0xe6, 0x1f, 0xab, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_06() {
		...
		unit_test_helper(
			&[0x20, 0x0a, 0x2d, 0x1e, 0xe1, 0x00],
			&[0x38, 0x7b, 0xcf, 0x1e, 0xe1, 0x5b, 0x3c, 0x6b, 0x9e, 0xb4, 0x0a, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_07() {
		...
		unit_test_helper(
			&[0x2b, 0x82, 0xc3, 0x0b, 0x21, 0xfa, 0x00],
			&[0x82, 0x45, 0xc3, 0xfd, 0x0b, 0xb8, 0x96, 0xa5, 0x49, 0x21, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_08() {
		...
		unit_test_helper(
			&[0x0a, 0x84, 0xd2, 0xdd, 0x00],
			&[0xdd, 0x84, 0xbc, 0xd4, 0x0c, 0xa5, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_10() {
		...
		unit_test_helper(
			&[0x20, 0x20, 0xea, 0x20, 0x4a, 0x00],
			&[0x20, 0x4d, 0x32, 0xea, 0x4a, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_11() {
		...
		unit_test_helper(
			&[0x43, 0x2b, 0xbd, 0x00],
			&[0xbd, 0x98, 0xd4, 0x2a, 0x16, 0x97, 0x43, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_12() {
		// invalid base: contains 0x2b ('+')
		unit_test_helper(&[0x2d, 0x2b, 0xce, 0x00], &[0xce, 0x2b, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_13() {
		// invalid base: contains 0x2d ('-')
		unit_test_helper(
			&[0x20, 0x2b, 0x2c, 0x2d, 0x00],
			&[0x2c, 0x2e, 0x2d, 0x65, 0x30, 0xcf, 0xbc, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_14() {
		...
		unit_test_helper(&[0x0b, 0x2b, 0x89, 0x2f, 0x00], &[0xcd, 0x89, 0x2f, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_15() {
		// s: empty string
		unit_test_helper(&[0x00], &[0xb6, 0x3d, 0x21, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_16() {
		// s: only whitespace(s)
		unit_test_helper(
			&[0x20, 0x0c, 0x09, 0x20, 0x0a, 0x0a, 0x0b, 0x0d, 0x00],
			&[0xf1, 0x3c, 0x96, 0xc9, 0xa8, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_17() {
		// s: only 0x2b ('+')
		unit_test_helper(&[0x2b, 0x00], &[0xb2, 0x99, 0x07, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_18() {
		// s: only 0x2d ('-')
		unit_test_helper(&[0x2d, 0x00], &[0x46, 0x4f, 0x30, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_19() {
		...
		unit_test_helper(&[0x2b, 0x2b, 0x5b, 0x5d, 0x00], &[0x5b, 0x5d, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_20() {
		...
		unit_test_helper(&[0x2d, 0x2d, 0x92, 0x00], &[0x76, 0x77, 0x92, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_21() {
		...
		unit_test_helper(&[0x2b, 0x2d, 0x41, 0x41, 0x00], &[0x5a, 0x41, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_22() {
		...
		unit_test_helper(&[0x2d, 0x20, 0x13, 0x91, 0x00], &[0x91, 0x13, 0x00], 0);
	}
	// endregion
	...
	fn ft_atoi_base_23() {
		...
		unit_test_helper(
			&[0xbd, 0xbd, 0xbd, 0xbd, 0xbd, 0xbd, 0xbd, 0xbd, 0x00],
			&[0xbd, 0x02, 0x8c, 0xac, 0x95, 0x8e, 0x2a, 0x16, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_24() {
		...
		unit_test_helper(
			&[0xbd, 0xbd, 0x0c, 0x20, 0x09, 0x16, 0x22, 0xbd, 0x00],
			&[0xbd, 0x02, 0x8c, 0xac, 0x95, 0x8e, 0x22, 0x16, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_25() {
		...
		unit_test_helper(
			&[0xe3, 0xe3, 0x2b, 0x05, 0xf6, 0x16, 0x00],
			&[0xe3, 0x16, 0xf6, 0x96, 0xa2, 0x51, 0xaf, 0x49, 0x05, 0x5e, 0x1f, 0x61, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_26() {
		...
		unit_test_helper(
			&[0xc4, 0xc4, 0xc4, 0xc4, 0x2d, 0xc9, 0xf4, 0xa3, 0x00],
			&[0xc4, 0xc3, 0xf4, 0x69, 0x18, 0x96, 0xc9, 0x45, 0xa3, 0x5d, 0x00],
			0,
		);
	}
	// endregion
	...
	fn ft_atoi_base_27() {
		...
		unit_test_helper(
			&[0x70, 0x6f, 0x6e, 0x65, 0x79, 0x76, 0x69, 0x66, 0x00],
			&[0x65, 0x66, 0x69, 0x6e, 0x6f, 0x70, 0x76, 0x79, 0x00],
			11636625,
		);
	}
	// endregion
	...
	fn ft_atoi_base_28() {
		...
		unit_test_helper(
			&[0x0d, 0x0c, 0x0c, 0x0c, 0x20, 0xea, 0xea, 0xea, 0x83, 0xcb, 0x00],
			&[0xcb, 0xea, 0x83, 0x00],
			123,
		);
	}
	...
	fn ft_atoi_base_29() {
		...
		unit_test_helper(
			&[0x2b, 0x68, 0x21, 0x9d, 0x74, 0xe8, 0x00],
			&[0x8b, 0xb9, 0xe8, 0x21, 0x9a, 0x68, 0x35, 0xb8, 0x9d, 0x74, 0x34, 0x7a, 0x00],
			110126,
		);
	}
	// endregion
	...
	fn ft_atoi_base_30() {
		...
		unit_test_helper(
			&[0x2d, 0x42, 0x41, 0x42, 0x41, 0x42, 0x41, 0x00],
			&[0x41, 0x42, 0x00],
			-42,
		);
	}
	...
	fn ft_atoi_base_31() {
		...
		unit_test_helper(
			&[0xa6, 0xa6, 0xa6, 0xa6, 0xa6, 0xa8, 0xa8, 0x00],
			&[0xa6, 0x6f, 0xdf, 0xa7, 0xa8, 0x23, 0x00],
			28,
		);
	}
	// endregion
	...
	fn ft_atoi_base_32() {
		...
		unit_test_helper(
			&[0x09, 0x20, 0x0d, 0x0d, 0x2d, 0xba, 0xd6, 0x40, 0x4a, 0x5a, 0x2b, 0x19, 0x0a, 0x00],
			&[
	...
	fn ft_atoi_base_33() {
		...
		unit_test_helper(
			...
			&[0x56, 0x8b, 0xb6, 0x04, 0x76, 0x64, 0x57, 0xc1, 0x00],
			i32::MAX,
		);
	}
	// endregion
	...
	fn ft_atoi_base_34() {
		...
		unit_test_helper(
			&[
				...
				0x4a, 0x54, 0x54, 0x9f, 0x00,
			],
			&[0xd6, 0x7a, 0x0e, 0xdf, 0x4a, 0xba, 0x41, 0xd4, 0x91, 0x9a, 0x54, 0x9f, 0xc7, 0x00],
			i32::MIN,
		);
	}
	// endregion
	...
	fn ft_atoi_base_36() {
		...
		unit_test_helper(
			...
			&[0x69, 0xd5, 0x77, 0xd0, 0xa2, 0x8a, 0x0f, 0x07, 0x1f, 0x33, 0x00],
			-1549156548,
		);
	}
	// endregion
