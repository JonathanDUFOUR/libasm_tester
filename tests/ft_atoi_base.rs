#[cfg(test)]
mod tests {
	use std::ffi::{c_char, c_int};

	#[link(name = "asm_bonus")]
	extern "C" {
		fn ft_atoi_base(s: *const c_char, base: *const c_char) -> c_int;
	}

	#[inline(always)]
	fn helper(s: &[u8], base: &[u8], expected: i32) {
		use std::ffi::CStr;

		let s: &CStr = CStr::from_bytes_with_nul(s).unwrap();
		let base: &CStr = CStr::from_bytes_with_nul(base).unwrap();

		assert_eq!(unsafe { ft_atoi_base(s.as_ptr(), base.as_ptr()) }, expected);
	}

	// region: ft_atoi_base_00
	#[test]
	fn ft_atoi_base_00() {
		// invalid base: empty string
		helper(&[0xA6, 0xEC, 0x9D, 0x4C, 0xA2, 0xC4, 0xD0, 0x00], &[0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_01
	#[test]
	fn ft_atoi_base_01() {
		// invalid base: length < 2
		helper(&[0x20, 0x01, 0x7D, 0xA2, 0xB3, 0x00], &[0x01, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_02
	#[test]
	fn ft_atoi_base_02() {
		// invalid base: contains duplicates
		helper(
			&[0x56, 0x7E, 0xB1, 0x9A, 0xD0, 0x80, 0x90, 0x6A, 0x00],
			&[0x95, 0x4B, 0x56, 0x95, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_03
	#[test]
	fn ft_atoi_base_03() {
		// invalid base: contains 0x2B ('+')
		helper(&[0x2B, 0x3B, 0xC6, 0xDD, 0x00], &[0xC6, 0x3B, 0x45, 0x2B, 0x9B, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_04
	#[test]
	fn ft_atoi_base_04() {
		// invalid base: contains 0x2D ('-')
		helper(&[0x0C, 0x0C, 0x2B, 0xA3, 0x20, 0x00], &[0xB6, 0x2D, 0xB7, 0xA8, 0xA3, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_05
	#[test]
	fn ft_atoi_base_05() {
		// invalid base: contains 0x09 ('\t')
		helper(
			&[0x09, 0x0C, 0xA1, 0x00],
			&[0x09, 0xA1, 0xD0, 0xFC, 0xDA, 0xE6, 0x1F, 0xAB, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_06
	#[test]
	fn ft_atoi_base_06() {
		// invalid base: contains 0x0A ('\n')
		helper(
			&[0x20, 0x0A, 0x2D, 0x1E, 0xE1, 0x00],
			&[0x38, 0x7B, 0xCF, 0x1E, 0xE1, 0x5B, 0x3C, 0x6B, 0x9E, 0xB4, 0x0A, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_07
	#[test]
	fn ft_atoi_base_07() {
		// invalid base: contains 0x0B ('\v')
		helper(
			&[0x2B, 0x82, 0xC3, 0x0B, 0x21, 0xFA, 0x00],
			&[0x82, 0x45, 0xC3, 0xFD, 0x0B, 0xB8, 0x96, 0xA5, 0x49, 0x21, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_08
	#[test]
	fn ft_atoi_base_08() {
		// invalid base: contains 0x0C ('\f')
		helper(&[0x0A, 0x84, 0xD2, 0xDD, 0x00], &[0xDD, 0x84, 0xBC, 0xD4, 0x0C, 0xA5, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_09
	#[test]
	fn ft_atoi_base_09() {
		// invalid base: contains 0x0D ('\r')
		helper(
			&[0xCE, 0xAA, 0x0D, 0x2D, 0x7D, 0x00],
			&[
				0xE0, 0x02, 0x61, 0x78, 0x7D, 0x8B, 0xDE, 0x74, 0xE1, 0x24, 0xA5, 0x0D, 0x86, 0xAA,
				0x9C, 0xCE, 0x00,
			],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_10
	#[test]
	fn ft_atoi_base_10() {
		// invalid base: contains 0x20 (' ')
		helper(&[0x20, 0x20, 0xEA, 0x20, 0x4A, 0x00], &[0x20, 0x4D, 0x32, 0xEA, 0x4A, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_11
	#[test]
	fn ft_atoi_base_11() {
		// invalid base: contains 0x2A ('*')
		helper(&[0x43, 0x2B, 0xBD, 0x00], &[0xBD, 0x98, 0xD4, 0x2A, 0x16, 0x97, 0x43, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_12
	#[test]
	fn ft_atoi_base_12() {
		// invalid base: contains 0x2B ('+')
		helper(&[0x2D, 0x2B, 0xCE, 0x00], &[0xCE, 0x2B, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_13
	#[test]
	fn ft_atoi_base_13() {
		// invalid base: contains 0x2D ('-')
		helper(
			&[0x20, 0x2B, 0x2C, 0x2D, 0x00],
			&[0x2C, 0x2E, 0x2D, 0x65, 0x30, 0xCF, 0xBC, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_14
	#[test]
	fn ft_atoi_base_14() {
		// invalid base: contains 0x2F ('/')
		helper(&[0x0B, 0x2B, 0x89, 0x2F, 0x00], &[0xCD, 0x89, 0x2F, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_15
	#[test]
	fn ft_atoi_base_15() {
		// s: empty string
		helper(&[0x00], &[0xB6, 0x3D, 0x21, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_16
	#[test]
	fn ft_atoi_base_16() {
		// s: only whitespace(s)
		helper(
			&[0x20, 0x0C, 0x09, 0x20, 0x0A, 0x0A, 0x0B, 0x0D, 0x00],
			&[0xF1, 0x3C, 0x96, 0xC9, 0xA8, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_17
	#[test]
	fn ft_atoi_base_17() {
		// s: only 0x2B ('+')
		helper(&[0x2B, 0x00], &[0xB2, 0x99, 0x07, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_18
	#[test]
	fn ft_atoi_base_18() {
		// s: only 0x2D ('-')
		helper(&[0x2D, 0x00], &[0x46, 0x4F, 0x30, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_19
	#[test]
	fn ft_atoi_base_19() {
		// s: contains more than 1 0x2B ('+') before the number
		helper(&[0x2B, 0x2B, 0x5B, 0x5D, 0x00], &[0x5B, 0x5D, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_20
	#[test]
	fn ft_atoi_base_20() {
		// s: contains more than 1 0x2D ('-') before the number
		helper(&[0x2D, 0x2D, 0x92, 0x00], &[0x76, 0x77, 0x92, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_21
	#[test]
	fn ft_atoi_base_21() {
		// s: contains both 0x2B ('+') and 0x2D ('-') before the number
		helper(&[0x2B, 0x2D, 0x41, 0x41, 0x00], &[0x5A, 0x41, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_22
	#[test]
	fn ft_atoi_base_22() {
		// s: contains whitespace(s) between the sign and the number
		helper(&[0x2D, 0x20, 0x13, 0x91, 0x00], &[0x91, 0x13, 0x00], 0);
	}
	// endregion

	// region: ft_atoi_base_23
	#[test]
	fn ft_atoi_base_23() {
		// s: contains only null digit(s)
		helper(
			&[0xBD, 0xBD, 0xBD, 0xBD, 0xBD, 0xBD, 0xBD, 0xBD, 0x00],
			&[0xBD, 0x02, 0x8C, 0xAC, 0x95, 0x8E, 0x2A, 0x16, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_24
	#[test]
	fn ft_atoi_base_24() {
		// s: contains whitespaces between the leading null digit(s) and the signicant digit(s)
		helper(
			&[0xBD, 0xBD, 0x0C, 0x20, 0x09, 0x16, 0x22, 0xBD, 0x00],
			&[0xBD, 0x02, 0x8C, 0xAC, 0x95, 0x8E, 0x22, 0x16, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_25
	#[test]
	fn ft_atoi_base_25() {
		// s: contains 0x2B ('+') between the leading null digit(s) and the signicant digit(s)
		helper(
			&[0xE3, 0xE3, 0x2B, 0x05, 0xF6, 0x16, 0x00],
			&[0xE3, 0x16, 0xF6, 0x96, 0xA2, 0x51, 0xAF, 0x49, 0x05, 0x5E, 0x1F, 0x61, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_26
	#[test]
	fn ft_atoi_base_26() {
		// s: contains 0x2D ('-') between the leading null digit(s) and the signicant digit(s)
		helper(
			&[0xC4, 0xC4, 0xC4, 0xC4, 0x2D, 0xC9, 0xF4, 0xA3, 0x00],
			&[0xC4, 0xC3, 0xF4, 0x69, 0x18, 0x96, 0xC9, 0x45, 0xA3, 0x5D, 0x00],
			0,
		);
	}
	// endregion

	// region: ft_atoi_base_27
	#[test]
	fn ft_atoi_base_27() {
		// s: contains only significant digit(s)
		helper(
			&[0x70, 0x6F, 0x6E, 0x65, 0x79, 0x76, 0x69, 0x66, 0x00],
			&[0x65, 0x66, 0x69, 0x6E, 0x6F, 0x70, 0x76, 0x79, 0x00],
			11636625,
		);
	}
	// endregion

	// region: ft_atoi_base_28
	#[test]
	fn ft_atoi_base_28() {
		// s: contains leading whitespace(s) and significant digit(s)
		helper(
			&[0x0D, 0x0C, 0x0C, 0x0C, 0x20, 0xEA, 0xEA, 0xEA, 0x83, 0xCB, 0x00],
			&[0xCB, 0xEA, 0x83, 0x00],
			123,
		);
	}
	// endregion

	// region: ft_atoi_base_29
	#[test]
	fn ft_atoi_base_29() {
		// s: contains leading 0x2B ('+') and significant digit(s)
		helper(
			&[0x2B, 0x68, 0x21, 0x9D, 0x74, 0xE8, 0x00],
			&[0x8B, 0xB9, 0xE8, 0x21, 0x9A, 0x68, 0x35, 0xB8, 0x9D, 0x74, 0x34, 0x7A, 0x00],
			110126,
		);
	}
	// endregion

	// region: ft_atoi_base_30
	#[test]
	fn ft_atoi_base_30() {
		// s: contains leading 0x2D ('-') and significant digit(s)
		helper(&[0x2D, 0x42, 0x41, 0x42, 0x41, 0x42, 0x41, 0x00], &[0x41, 0x42, 0x00], -42);
	}
	// endregion

	// region: ft_atoi_base_31
	#[test]
	fn ft_atoi_base_31() {
		// s: contains leading null digit(s) and significant digit(s)
		helper(
			&[0xA6, 0xA6, 0xA6, 0xA6, 0xA6, 0xA8, 0xA8, 0x00],
			&[0xA6, 0x6F, 0xDF, 0xA7, 0xA8, 0x23, 0x00],
			28,
		);
	}
	// endregion

	// region: ft_atoi_base_32
	#[test]
	fn ft_atoi_base_32() {
		// s: contains leading whitespace(s), 0x2D ('-') and significant digit(s)
		helper(
			&[0x09, 0x20, 0x0D, 0x0D, 0x2D, 0xBA, 0xD6, 0x40, 0x4A, 0x5A, 0x2B, 0x19, 0x0A, 0x00],
			&[
				0x40, 0xFF, 0xBA, 0x76, 0xB9, 0x19, 0x5A, 0xD6, 0xE0, 0xF8, 0x92, 0x89, 0xF0, 0x4A,
				0x97, 0x00,
			],
			-125076,
		);
	}
	// endregion

	// region: ft_atoi_base_33
	#[test]
	fn ft_atoi_base_33() {
		// s: represents i32::MAX
		helper(
			&[
				0x0A, 0x20, 0x56, 0x8B, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1, 0xC1,
				0x00,
			],
			&[0x56, 0x8B, 0xB6, 0x04, 0x76, 0x64, 0x57, 0xC1, 0x00],
			i32::MAX,
		);
	}
	// endregion

	// region: ft_atoi_base_34
	#[test]
	fn ft_atoi_base_34() {
		// s: represents i32::MIN
		helper(
			&[
				0x0D, 0x0D, 0x20, 0x0A, 0x2D, 0x0D6, 0xD6, 0xD6, 0x0E, 0x91, 0x0E, 0x9F, 0x54,
				0x4A, 0x54, 0x54, 0x9F, 0x00,
			],
			&[0xD6, 0x7A, 0x0E, 0xDF, 0x4A, 0xBA, 0x41, 0xD4, 0x91, 0x9A, 0x54, 0x9F, 0xC7, 0x00],
			i32::MIN,
		);
	}
	// endregion

	// region: ft_atoi_base_35
	#[test]
	fn ft_atoi_base_35() {
		// s: represents a number greater than i32::MAX
		helper(
			&[
				0x20, 0x20, 0x0B, 0x2B, 0x76, 0x13, 0x01, 0x01, 0xE0, 0x76, 0xAA, 0x76, 0x13, 0x80,
				0x00,
			],
			&[
				0x76, 0x13, 0x80, 0xE4, 0x44, 0x5C, 0x0F, 0x93, 0x39, 0x23, 0xE0, 0x81, 0x63, 0xAA,
				0x8A, 0x01, 0x00,
			],
			-6238190,
		);
	}
	// endregion

	// region: ft_atoi_base_36
	#[test]
	fn ft_atoi_base_36() {
		// s: represents a number lower than i32::MIN
		helper(
			&[
				0x09, 0x09, 0x09, 0x0C, 0x2D, 0x33, 0x33, 0xD0, 0x0F, 0x1F, 0x0F, 0x0F, 0x69, 0xD5,
				0x33, 0x77, 0xA2, 0x00,
			],
			&[0x69, 0xD5, 0x77, 0xD0, 0xA2, 0x8A, 0x0F, 0x07, 0x1F, 0x33, 0x00],
			-1549156548,
		);
	}
	// endregion
}
